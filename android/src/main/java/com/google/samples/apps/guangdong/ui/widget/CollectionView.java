...
package com.google.samples.apps.guangdong.ui.widget;
...
public class CollectionView extends ListView {
    ...
    public void setContentTopClearance(int clearance) {
        if (mContentTopClearance != clearance) {
            ...
            //Log.d("dongdong","clearance"+Integer.toString(clearance));
    ...
    private boolean computeRowContent(int row, RowComputeResult result) {
        ...
        for (InventoryGroup group : mInventory.mGroups) {
            ...
            while (posInGroup < group.mItemCount) {
                ...
                // advance to next row
                posInGroup += group.mDisplayCols;/** 每次都加上每行的列数 */
                curRow++;
            }
        }
        return false;
    ...
    /** 创建RowView，包括Header、CustomGroupView、Item */
    ...
    /** 创建不是Header的View，即CustomView和普通Item；
     * 分为新建和回收 */
    ...
    /** 从开头重新走，将Header与普通Item全都加入GroupView的getChildAt(0)的LinearLayout中 */
    private View createGroupView(RowComputeResult rowInfo, View view, ViewGroup parent) {
        ...
        /** 利用普通Header与Item的创建方式填充groupViewContent */
    ...
    /** 可以分别创建CustomGroupView和普通瀑布流Item所对应的列 */
    private View getItemView(RowComputeResult rowInfo, int column, View view, ViewGroup parent) {
        ...
        /** 利用Callbacks分别创建和绑定ItemView，到此创建Item结束
         * 回收的时候只执行绑定
         * */
        if (view == null || view instanceof EmptyView) {
    ...
    /** 分别回收CustomGroupView和Item中的View */
    ...
    public static class InventoryGroup implements Cloneable {
        ...
        /** 利用现有InventoryGroup创建新的InventoryGroup */
