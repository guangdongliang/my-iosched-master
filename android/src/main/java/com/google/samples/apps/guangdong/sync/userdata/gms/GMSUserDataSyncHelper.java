...
import com.google.samples.apps.guangdong.sync.userdata.util.UserDataHelper;
...
public class GMSUserDataSyncHelper extends AbstractUserDataSyncHelper {
     ...
     * 对比remoteActions与actions，不同则同步，相同则返回
     */
    @Override
    protected boolean syncImpl(List<UserAction> actions, boolean hasPendingLocalData) {
        try {
            ...
            * are up-to-date.
            ...
            /** 对比localGcmKey与remoteGcmKey，然后判断是否更新本地GcmKey */
    ...
    /** 无法后台同步，I/O应用程序需要权限，I/O应用程序需要完成后台同步。 */
    ...
    private void setupApiClient() {
        mGoogleApiClient = new GoogleApiClient.Builder(mContext)
                .addApi(Drive.API) /** 可以用addApiIfAvailable，防止API不存在出现错误 */
                .addScope(Drive.SCOPE_APPFOLDER)
                .setAccountName(mAccountName)
                .build();
    }
    /** 利用helper.getContentsFromDrive(DriveId)从服务器上获取数据 */
