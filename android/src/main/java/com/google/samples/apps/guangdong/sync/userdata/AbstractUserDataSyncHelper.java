...
package com.google.samples.apps.guangdong.sync.userdata;
...
public abstract class AbstractUserDataSyncHelper {
     ...
     * 查找MySchedule、MyViewedVideos、MyFeedbackSubmitted中的anction，合并到actions中。
     * 再用上边的syncImpl处理，
     * 最后notify改变
     */
    public boolean sync() {
        ...
        if (scheduleData != null) {
            while (scheduleData.moveToNext()) {
                ...
                userAction.requiresSync = scheduleData.getInt(
                        scheduleData.getColumnIndex(MySchedule.MY_SCHEDULE_DIRTY_FLAG)) == 1;/** 需要同步的item */
    ...
    /** 将actions中的所有item的dirty设置成0，即不需要更新 */
    private void resetDirtyFlag(ArrayList<UserAction> actions) {
        ...
        for (UserAction action : actions) {
            ...
            if (action.type == UserAction.TYPE.VIEW_VIDEO) {
                ...
                dirtyField = MyViewedVideos.MY_VIEWED_VIDEOS_DIRTY_FLAG;/** 是否需要更新的Column */
                ...
                dirtyField = MyFeedbackSubmitted.MY_FEEDBACK_SUBMITTED_DIRTY_FLAG;/** 是否需要更新的Column */
            } else {
                ...
                dirtyField = MySchedule.MY_SCHEDULE_DIRTY_FLAG;/** 是否需要更新的Column */
            }
            ...
            ContentProviderOperation op = ContentProviderOperation.newUpdate(
                    ...
                    .withValue(dirtyField, 0)/** 将需要更新的Column全都设置为0 */
                    .build();
