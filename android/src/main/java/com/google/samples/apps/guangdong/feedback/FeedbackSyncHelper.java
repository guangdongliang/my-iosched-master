...
public class FeedbackSyncHelper {
    ...
    /** 将还未更新的评论上传到服务器上，然后将数据库中的同步值：synced 改成1 */
    public void sync() {
        ...
        try {
            while (c.moveToNext()) {
                ...
                questions.put(
                        QUESTION_KEYS.get(ScheduleContract.Feedback.SESSION_RATING),/** 获得"Q10" */
                        RATING_ANSWERS.get(data));/** 根据评级获得长字符串 */
                /** 第二个，相关性评级 */
                data = c.getString(c.getColumnIndex(ScheduleContract.Feedback.ANSWER_RELEVANCE));
                questions.put(
                        QUESTION_KEYS.get(ScheduleContract.Feedback.ANSWER_RELEVANCE),/** 获得"Q20" */
                        RELEVANCE_ANSWERS.get(data));/** 根据相关答案（整数）获得长字符串 */
                /** 内容评级 */
                data = c.getString(c.getColumnIndex(ScheduleContract.Feedback.ANSWER_CONTENT));
                questions.put(
                        QUESTION_KEYS.get(ScheduleContract.Feedback.ANSWER_CONTENT),/** 获得Q30 */
                        CONTENT_ANSWERS.get(data));/** 根据内容（整数）获得长字符串 */
                /** speaker评级 */
                data = c.getString(c.getColumnIndex(ScheduleContract.Feedback.ANSWER_SPEAKER));
                questions.put(
                        QUESTION_KEYS.get(ScheduleContract.Feedback.ANSWER_SPEAKER),/** 获得Q40 */
                        SPEAKER_ANSWERS.get(data));/** 根据Speaker（整数）获得长字符串 */
                /** commit评级 */
                data = c.getString(c.getColumnIndex(ScheduleContract.Feedback.COMMENTS));
                questions.put(
                        QUESTION_KEYS.get(ScheduleContract.Feedback.COMMENTS),/** 获得Q40 */
                        data);
