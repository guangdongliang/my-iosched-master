...
public class UserDataHelper {
     ...
     * 通过Gson将userData变成String，在转换成bytes
     */
     ...
     * 利用actions填满UserData中的3个集合，即创建UserData
     */
     ...
     * Array.
     * 从 queryUri的database的column列中取出数据并加入Set(String)中
     */
     ...
     * 分别从DB中取出MySchedule.SESSION_ID、MyViewedVideos.VIDEO_ID、MyFeedbackSubmitted.SESSION_ID列，并分别加入UserData的Set中
     */
     ...
     * 这是发现remote与local不同的时候，先用他两个生成merge，然后再用此函数将userData写入数据库中
     * 先删除DB上MySchedule、MyViewedVideos、MyFeedbackSubmitted 数据，然后利用userData重新创建
     */
