...
public class SpeakersHandler extends JSONHandler {
    ...
    public void makeContentProviderOperations(ArrayList<ContentProviderOperation> list) {
        ...
        for (Speaker speaker : mSpeakers.values()) {
            ...
            /** 条件：Speaker数据库为空、不为空但是不包含speaker.id、不为空且包含speaker.id但hashCode不相等 */
        ...
        /**
         * 如果从数据库中提取出来的speakerHashcodes中含有speakersToKeep不包含的元素，剔除
         */
        if (isIncrementalUpdate) {
    ...
    /** 建立删除本SpeakerId 的 ContentProviderOperation */
    ...
    /** 从DB中读取Speaker数据，产生HashMap，<speakerId,SpeakerHashcodes> */
