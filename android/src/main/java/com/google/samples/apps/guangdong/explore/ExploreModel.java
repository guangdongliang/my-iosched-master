 ...
 * {@link com.google.samples.apps.guangdong.framework.PresenterFragmentImpl}.
 */
public class ExploreModel implements Model {
    ...
    /** 如果query为ExploreQueryEnum.SESSIONS，则利用cursor填充mKeynoteData、mThemes、mTopics、mLiveStreamData
     * 如果query为ExploreQueryEnum.TAGS，则利用cursor填充mTagTitles */
    @Override
        ...
        if (query == ExploreQueryEnum.SESSIONS) {
            ...
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    ...
                    /** 如果是 KEYNOTE 重新填充SessionData并更改其Details */
                    if (Config.Tags.SPECIAL_KEYNOTE.equals(session.getMainTag())) {
                        ...
                        /** 如果是现场直播，将其加入liveStreamData中 */
                        liveStreamData.addSessionData(session);
                    }

                    // TODO: Refactor into a system wide way of parsing these tags.
                    if (!TextUtils.isEmpty(tags)) {
                        /** 如果tags中含有TOPIC_ 则将session加入topicGroups中
                         * 如果tags中含有THEME_ 则将session加入themeGroup中 */
            ...
            /** 最终得到mThemes、mTopics和mLiveStreamData */
    ...
    /** 根据是否出席将TopicSessionLimit设为：2  0 */
    ...
    /** 根据是否出席得出themeSessionLimit的值： 2  6 */
    public static int getThemeSessionLimit(Context context) {
        ...
        if (atVenue) {
            themeSessionLimit = context.getResources().getInteger(R.integer
                    .explore_topic_theme_onsite_max_item_count);/** 2 */
        } else {
            themeSessionLimit = context.getResources().getInteger(R.integer
                    .explore_theme_max_item_count_offsite);/** 6 */
        }
        return themeSessionLimit;
    }
    /** 如果是现场直播：将Details改为LIVE NOW，否则：将Details改为startTime */
    ...
    /** 根据cursor中的列信息，重新设置session的各属性信息 */
    ...
    /** 根据loaderId的值选择建立SESSIONS或TAGS的CursorLoader */
    @Override
