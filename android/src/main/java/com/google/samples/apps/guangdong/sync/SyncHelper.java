...
import com.google.samples.apps.guangdong.feedback.FeedbackApiHelper;
import com.google.samples.apps.guangdong.feedback.FeedbackSyncHelper;
...
import com.google.samples.apps.guangdong.sync.userdata.AbstractUserDataSyncHelper;
import com.google.samples.apps.guangdong.sync.userdata.UserDataSyncHelperFactory;
...
public class SyncHelper {
    ...
    /**
     * 本函数将调用下边的performSync进行同步
     * @param mChosenAccount
     * @param userDataSyncOnly
     */
     ...
     * 利用mRemoteDataFetcher从服务器获取数据，并利用mConferenceDataHandler更新本地DB,在设置更新成功
    ...
    private boolean doConferenceDataSync() throws IOException {
        ...
        if (dataFiles != null) {
            ...
            /** 将从RemoteDataFetcher.fetchConfenceDataIfNewer()获得的服务器数据，保存在数据库中 */
   ...
   private static long calculateRecommendedSyncInterval(final Context context) {
    ...
    public static void updateSyncInterval(final Context context, final Account account) {
        ...
        if (recommended != current) {
            ...
            if (recommended <= 0L) { /** Disable periodic sync.会议结束，取消DB同步 */
