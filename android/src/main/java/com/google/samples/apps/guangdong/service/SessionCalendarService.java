...
public class SessionCalendarService extends IntentService {
    ...
    /**
     * 处理ACTION_UPDATE_ALL_SESSIONS_CALENDAR 与 ACTION_CLEAR_ALL_SESSIONS_CALENDAR事件
     * @param intent
     */
    @Override
     ...
     * 根据intent携带的accountName信息，从数据库中取出CalendarId
     */
     ...
     * {@link com.google.samples.apps.guangdong.provider.ScheduleProvider}, adding or removing
    ...
    private ArrayList<ContentProviderOperation> processAllSessionsCalendar(ContentResolver resolver,
        ...
        * remove from Calendar if unstarred. */
        /** 检索Schedule.Sessions.CONTENT_URI,如果已经加星则加入Calendar中、未加星则从Calendar中剔除 */
     ...
     * 根据isAddEvent确定添加Calendar event还是删除
     */
    private ArrayList<ContentProviderOperation> processSessionCalendar(
        ...
        if (isAddEvent) {
            ...
            // duplicate one.
            /** 查询CalendarContract中时间在Config.CONFERENCE_START_MILLIS与Config.CONFERENCE_END_MILLIS之间的事件 */
            ...
            if (cursor != null && cursor.moveToFirst()) {
                 ...
                 *  在本时间段内的_ID不存在，创建之，并立即得到创建过的_ID
     ...
     * 删除所有id=calenderId、发生时间在CONFERENCE_START_MILLIS与CONFERENCE_END_MILLIS之间的事件
     */
    private ArrayList<ContentProviderOperation> processClearAllSessions(
        ...
        /** Like关键字用来模糊比较字符串，百分号%匹配0个或多个字符，下划线'_'匹配一个字符。
         * 还可以用方括号只匹配指定的某几个字符，例如[a-z]，匹配小写字面。方括号中加入^表示匹配除指定字符以外的字符。
         */
